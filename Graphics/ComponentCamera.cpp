...
{
	vec3 directionToOrigin = componentParent->GetWorldSpacePosition();
	float lengthToOrigin = glm::length(directionToOrigin);
	frustum.nearFace.normal = componentParent->forward;
	frustum.nearFace.distance = nearClip + (lengthToOrigin * glm::dot(glm::normalize(directionToOrigin), componentParent->forward));
...
bool ComponentCamera::IsPointInFrustum(vec3 point, float forgiveness)
{
	// Check near plane
